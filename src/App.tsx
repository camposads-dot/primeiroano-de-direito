...
const Hero = () => (
  <section className="relative min-h-[85vh] flex flex-col pt-0 pb-12 overflow-hidden">
    ...
    <header className="h-10 border-b border-white/10 flex items-center px-4 md:px-8 justify-between bg-black/40 backdrop-blur-md mb-8 md:mb-16">
      ...
      <div className="text-[10px] font-mono text-zinc-500 hidden sm:block">FALTAM: 02:45:12</div>
    </header>

    <div className="container mx-auto max-w-6xl px-4 flex-grow flex items-center">
      <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center w-full">
        <div className="order-2 lg:order-1">
          <motion.h1
            ...
            transition={{ delay: 0.1 }}
            className="text-4xl md:text-6xl font-black leading-[1.1] md:leading-[0.95] mb-4 tracking-tighter"
          >
            PARE DE SE <span className="text-brand-red">PERDER</span> NO DIREITO.
          </motion.h1>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            <Button className="w-full sm:w-auto px-6 py-3.5 text-base">
              QUERO MEU ACESSO <ArrowRight size={18} />
            </Button>
            <div className="flex items-center gap-2">
              <div className="flex -space-x-2">
                {[1,2,3,4].map(i => (
                  <img key={i} src={`https://picsum.photos/seed/user${i}/40/40`} className="w-7 h-7 rounded-full border border-white/20" referrerPolicy="no-referrer" />
                ))}
              </div>
              <span className="text-[9px] font-bold uppercase tracking-widest text-zinc-500">MÉTODO TESTADO</span>
            </div>
          </motion.div>

          <div className="mt-6 p-3 bg-zinc-900/50 border border-white/5 rounded-xl flex items-center justify-between max-w-xs">
             <div className="text-zinc-500 line-through text-xs font-bold uppercase tracking-widest">R$ 157,90</div>
             <div className="text-xl font-black text-brand-red uppercase tracking-tight">Combo: R$ 57,90</div>
          </div>
        </div>
        ...
        >
          <div className="aspect-video rounded-xl overflow-hidden glass relative border border-white/10 group shadow-2xl">
            <img 
              src="https://picsum.photos/seed/lawvideo/1280/720" 
              alt="Video Preview" 
              className="w-full h-full object-cover opacity-70 transition-transform duration-700" 
              referrerPolicy="no-referrer"
            />
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="w-14 h-14 bg-white text-black rounded-full flex items-center justify-center shadow-2xl group-hover:scale-110 transition-transform cursor-pointer">
                <Play fill="currentColor" size={20} className="ml-1" />
              </div>
            </div>
            <div className="absolute bottom-3 left-3">
              <p className="text-[9px] font-bold uppercase tracking-[0.2em] text-white bg-black/60 px-2 py-1 rounded">Domine o Direito em 7 dias</p>
            </div>
          </div>
        </motion.div>
      </div>
